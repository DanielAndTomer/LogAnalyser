...
def reformeUPSTemp(s):
    date_time=find_between(s,"] "," [")
    date=date_time[0:10]
    time=date_time[10:-1]
    temp=find_between(s,"maxTemp=",",")
    l=[date,time,temp]
    return l
...
##--------START ACTION: Browsing logs folder and setting output folders---------##
    ...
    #UPStemp csv file
    csvUPStemp = open(os.path.join(csvdir,"UPS Temp.csv"),'w')
    wr_ups = csv.writer(csvUPStemp, dialect='excel',lineterminator='\n')
    csvUPStemp.write("Date, Time, Max Temp\n")
        ...
        if os.path.isdir(path)==False:
            ...
            for line in contant:
                ...
                if "2: MobilicomGetSystemStatusResponse" in line:
                    ...
                    csvcounter+=1
                elif "translatedMessage: UpsResponse" in line:
                    csvline=reformeUPSTemp(line)
                    wr_ups.writerow(csvline)
                    csvcounter+=1
